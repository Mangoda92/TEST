...
def set_offset(offset_x, offset_y, offset_z):
    ...
    # X축 옵셋
    write_register(OFFSET_X_H, (offset_x >> 8) & 0xFF)  # X 상위 8비트
    time.sleep(0.1)
    write_register(OFFSET_X_L, offset_x & 0xFF)  # X 하위 8비트
    time.sleep(0.1)

    # Y축 옵셋
    write_register(OFFSET_Y_H, (offset_y >> 8) & 0xFF)  # Y 상위 8비트
    time.sleep(0.1)
    write_register(OFFSET_Y_L, offset_y & 0xFF)  # Y 하위 8비트
    time.sleep(0.1)

    # Z축 옵셋
    write_register(OFFSET_Z_H, (offset_z >> 8) & 0xFF)  # Z 상위 8비트
    time.sleep(0.1)
...
def read_acceleration(START_TIME):
    ...
    #print(f"DEVID_AD : {hex(spi.xfer2([DEVID_AD | 0x80, 0x00]))}")
