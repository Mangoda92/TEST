...
OUTPUT_FILE = "/home/mango/workspace/FFT_stream/test/data_log_2.csv"
...
def setup(START_TIME):
    ...
    # OFFSET 설정
    bus.write_byte_data(DEVICE_ADDR, OFSX, 5)  # X축 보정 2
    time.sleep(0.1)
    bus.write_byte_data(DEVICE_ADDR, OFSY, 70)  # Y축 보정 65
    time.sleep(0.1)
    bus.write_byte_data(DEVICE_ADDR, OFSZ, -10)  # Z축 보정 -18
    time.sleep(0.1)
    
    # ±2g 모드 설정 (Data Format register)
    bus.write_byte_data(DEVICE_ADDR, DATA_FORMAT, 0x00)  # ±2g (0x00) 설정
    time.sleep(0.1)
    
    # 샘플링 속도 설정 (800Hz)
    bus.write_byte_data(DEVICE_ADDR, BW_RATE, 0x0D)  # 800Hz
    time.sleep(0.1)

    # ADXL345을 측정 모드로 설정
    bus.write_byte_data(DEVICE_ADDR, POWER_CTL, 0x08)  # 측정 모드 활성화
    time.sleep(0.1)
...
def read_acceleration(START_TIME):
    ...
    X_LSB = bus.read_byte_data(DEVICE_ADDR, DATAX0)
    X_MSB = bus.read_byte_data(DEVICE_ADDR, DATAX1)
    Y_LSB = bus.read_byte_data(DEVICE_ADDR, DATAY0)
    Y_MSB = bus.read_byte_data(DEVICE_ADDR, DATAY1)
    Z_LSB = bus.read_byte_data(DEVICE_ADDR, DATAZ0)
    Z_MSB = bus.read_byte_data(DEVICE_ADDR, DATAZ1)
    ...
    print(f"{TIMESTAMP:.4f} s - X: {g_X:.4f} mg, Y: {g_Y:.4f} mg, Z: {g_Z:.4f} mg")
